...
import { experienceLevelIcons, getExperienceLevelTitle } from '../config/experience';
...
export const RetroExperienceCard = ({ experience }: RetroExperienceCardProps) => {
  ...
  const SkillMeter = ({ skill }: { skill: { name: string; level: number } }) => {
    return (
      <Box sx={{ mb: 1 }}>
      ...
      <CardContent>
        <Box sx={{ mb: 2 }}>
          ...
          <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1 }}>
            {experience.skills && experience.skills.map((skill, index) => (
              <SkillBadge key={index} skill={skill} color={theme.palette.accent2.main} />
            ))}
          </Stack>
        </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <EmojiEventsIcon sx={{ color: theme.palette.primary.main, fontSize: '1.2rem' }} />
            <Typography
              variant="caption"
              sx={{
                fontFamily: '"Press Start 2P", cursive',
