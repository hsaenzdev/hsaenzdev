...
export const Experience = () => {
  ...
  const loadMoreExperiences = () => {
    setVisibleExperiences(prevCount => Math.min(prevCount + 2, sortedExperiences.length));
  };
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 8, position: 'relative' }}>
      ...
      >
        ...
        <Box sx={{ mb: 6, position: 'relative', zIndex: 2 }}>
          ...
          >
            ...
            <GlowingText
              ...
              variant="h2"
              glowIntensity={15}
              sx={{ fontFamily: '"Press Start 2P", cursive', fontSize: { xs: '1.5rem', md: '2rem' } }}
            />
          </Box>
        ...
        {/* Experience Cards */}
        <Stack spacing={3}>
          {sortedExperiences.slice(0, visibleExperiences).map((experience, index) => {
            const animationDelay = hasAnimated ? 0 : index * 0.15 + 0.5;
            
            return (
              <Fade key={experience.id} in timeout={500} style={{ transitionDelay: `${index * 100}ms` }}>
                <Box>
                  <motion.div
